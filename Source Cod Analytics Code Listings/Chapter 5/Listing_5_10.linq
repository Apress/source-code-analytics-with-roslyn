...
var patternTokens = new List<SyntaxKind>()//#2
{
...
Func<List<string>, int, List<IEnumerable<string>>>

toNgrams = (list, n) =>
{
	List<int> ints2 =
		Enumerable.Range(1, list.Count).ToList();
	List<IEnumerable<string>> breaks2 =
	Enumerable.Range(0, ints2.Count / n + 1).ToList()
				.Select(z => list.Skip(z * n).Take(n))
				.Where(z => z.Count() > 0)
				.ToList();
...
var allPats = tree.GetRoot()
				.DescendantTokens()//#4
				.Where(t => patternTokens.Contains(t.Kind()))//#5
				.Select(t => t.ToFullString().Trim())//#6
				.Where(t => !t.Trim().Equals("{") && !t.Trim().Equals("}"))//#7
				.ToList();//#8

var dict = toNgrams(allPats, 2)//#9
			.ToLookup(t => t.Select(z => z.Trim())
			.Aggregate((a, b) => a + " " + b)) //#10
			.ToDictionary(t => t.Key, t => t.ToList().Count());//#11
			
dict.OrderByDescending(d => d.Value)
	.Where(d => d.Value >= 2)
	.Dump("Most frequent nested patterns");//#12